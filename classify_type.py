...
def main(argv=None):
    ...
    lambda_vec = 10**(-np.arange(1, 3, dtype=float))
    c_vec = 10**(np.arange(-1,1, dtype=float))
    ...
    for iSig in range(len(sigma_vec)):
        ...
        for iLam in range(len(lambda_vec)):
            ...
            # Compute Test Kernel
            kernel_test=kernel_fcn(x_test[0:200],x_train,params)
            for iC in range(len(c_vec)):
                ...
                #Test accuracy
                accuracies[iSig][iLam][iC]=model.score(kernel_test,y_test[0:200])
